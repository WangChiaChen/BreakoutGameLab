...
namespace BreakoutGameLab001
{
    internal class BrickGamePanel : Panel
    {
        ...
        public void Initialize()
        {
            // 初始化遊戲元件
            //
            balls.Add(new Ball(Width / 2, Height / 2, 15, 3, -3, Color.Orange));
            paddle = new Paddle(Width / 2 - 50, Height - 50, 120, 20, Color.Gray);
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    bricks.Add(new Brick(25 + j * 80, 25 + this.Location.Y + i * 30, 80, 30, Color.Yellow));
                }
            }
